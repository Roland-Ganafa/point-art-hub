...
const AdminProfile = () => {
  ...
  // Form state for creating new users - initialized with empty values
  ...
  const handleUpdateRole = async (profileId: string, newRole: "admin" | "user") => {
    if (profileId === currentProfile?.id && newRole !== "admin") {
      ...
      await logUserAction(
        ...
        "ROLE_CHANGE_DENIED",
        {
          ...
          reason: "Cannot remove own admin privileges"
        }
      );
    ...
    try {
      // Get the current profile data before updating
