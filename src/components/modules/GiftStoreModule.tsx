  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.selling_price) {
        itemData.selling_price = parseFloat(formData.selling_price);
      }
              ...
              <Table>
                ...
                <TableBody>
                  {filteredItems.map((item) => {
                    ...
                    const lowStockThreshold = item.low_stock_threshold !== undefined ? item.low_stock_threshold : 0;
                    ...
                    return (
                      <TableRow key={item.id} className={isLowStock ? "bg-red-50 dark:bg-red-900/20" : ""}>
                        ...
                        <TableCell className="font-semibold text-green-600">
                          {stock !== null ? formatUGX(stock) : "-"}
                        </TableCell>
                        <TableCell>
                          {sellingPrice !== null ? formatUGX(sellingPrice) : "-"}
                        </TableCell>
                        <TableCell>{new Date(item.date).toLocaleDateString()}</TableCell>
                        <TableCell>
                          {stock !== null && isLowStock ? (
